import { ArrowUpRight, CalendarDays, Clock3, Github, Quote, Sparkles } from 'lucide-react'
...
function prettyDate(value, lang) {
  if (!value || value === 'Current') return lang === 'fa' ? 'اکنون' : 'Current'
  const [year, month, day] = String(value).split('-').map(Number)
  if (!year) return value
  if (!month) return String(year)
  try {
    return new Intl.DateTimeFormat(lang === 'fa' ? 'fa-IR-u-ca-gregory' : 'en-US', { year:'numeric', month:'short', ...(day ? { day:'numeric' } : {}) }).format(new Date(Date.UTC(year, month - 1, day || 1)))
  } catch { return value }
}
...
export default function BlogCard({ article }) {
  ...
  const citations = Number.isFinite(livePaper?.citationCount) ? livePaper.citationCount : null
  return <article className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-glow dark:border-slate-800 dark:bg-slate-900">
    <Link to={href} className="block overflow-hidden bg-slate-100 dark:bg-slate-800">
      <img src={article.image} alt={`${article.title[lang]} — ${lang === 'fa' ? 'پژوهش پوریا ملکی' : 'research by Pouria Maleki'}`} loading="lazy" decoding="async" className={`aspect-[16/9] w-full transition duration-500 group-hover:scale-[1.035] ${article.imageFit === 'contain' ? 'object-contain p-4' : 'object-cover'}`} />
    </Link>
    <div className="p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500"><span className="inline-flex items-center gap-1"><CalendarDays size={14}/>{prettyDate(article.publishedDate || article.date, lang)}</span><span className="inline-flex items-center gap-1"><Clock3 size={14}/>{article.readTime} {t.blog.min}</span>{citations !== null && <span className="inline-flex items-center gap-1" title="Semantic Scholar citation count"><Quote size={13}/>{citations} {lang === 'fa' ? 'استناد' : 'citations'}</span>}{article.githubUrl && <span title="Related GitHub repository" className="inline-flex items-center gap-1"><Github size={14}/>GitHub</span>}</div>
      <h2 className="text-xl font-bold leading-snug tracking-tight md:text-2xl"><Link to={href}>{article.title[lang]}</Link></h2>
      {article.venue && <div className="mt-2 text-xs font-bold leading-5 text-slate-400">{article.venue}</div>}
