...
export default function BlogPost() {
  ...
  const livePaper = (liveData?.publications?.items || []).find(item => item.id === article.slug)
  const citationCount = Number.isFinite(livePaper?.citationCount) ? livePaper.citationCount : null
  ...
  return <><ReadingProgress/><SEO path={`/blog/${article.slug}`} title={`${title} | ${lang === 'fa' ? 'پوریا ملکی' : 'Pouria Maleki'}`} description={excerpt} image={article.image} type="article" article={article}/><main className="pb-20">
    ...
    <article className="container-shell mt-8"><header className="mx-auto max-w-4xl"><nav aria-label="Breadcrumb" className="mb-5 text-xs font-bold text-slate-400"><Link to={localizedPath('/',lang)}>{lang === 'fa' ? 'خانه' : 'Home'}</Link><span className="mx-2">/</span><Link to={localizedPath('/blog',lang)}>{lang === 'fa' ? 'وبلاگ' : 'Blog'}</Link></nav><div className="mb-5 flex flex-wrap gap-2 text-sm font-bold">{article.kind?.[lang] && <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-3 py-1 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300"><Sparkles size={13}/>{article.kind[lang]}</span>}<span className="rounded-full bg-blue-50 px-3 py-1 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{article.category[lang]}</span><span className="rounded-full bg-slate-100 px-3 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{displayDate}</span><span className="rounded-full bg-slate-100 px-3 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{article.readTime} {t.blog.min}</span></div><h1 className="text-4xl font-black leading-tight tracking-[-.04em] sm:text-5xl lg:text-6xl">{title}</h1><p className="mt-6 text-xl leading-9 text-slate-600 dark:text-slate-300">{excerpt}</p><div className="mt-6 space-y-2 text-sm text-slate-500"><div><span className="font-bold text-slate-700 dark:text-slate-300">{lang === 'fa' ? 'نویسندگان: ' : 'Authors: '}</span>{authors.join(' · ')}</div><div className="flex flex-wrap items-center gap-x-4 gap-y-2"><Link to={localizedPath('/about',lang)} className="font-black text-slate-800 hover:text-blue-600 dark:text-slate-200">{lang === 'fa' ? 'پوریا ملکی' : 'Pouria Maleki'}</Link>{article.venue && <span>{article.venue}</span>}{article.doi && <a href={`https://doi.org/${article.doi}`} target="_blank" rel="noreferrer" className="font-semibold hover:text-blue-600">DOI: {article.doi}</a>}{citationCount !== null && <span>{citationCount} {lang === 'fa' ? 'استناد · Semantic Scholar' : 'citations · Semantic Scholar'}</span>}{article.updated && <span>{lang === 'fa' ? 'به‌روزرسانی وبلاگ:' : 'Blog updated:'} {article.updated}</span>}</div></div></header>
    <div className="mx-auto mt-10 max-w-5xl overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900"><img src={article.image} alt={`${title} — ${lang === 'fa' ? 'مقاله پژوهشی پوریا ملکی' : 'research article by Pouria Maleki'}`} fetchPriority="high" decoding="async" className={`aspect-[16/7] w-full ${article.imageFit === 'contain' ? 'object-contain p-6' : 'object-cover'}`}/></div>
    ...
    {article.gallery?.length > 0 && <section className="mx-auto mt-8 max-w-5xl">
      <div className="mb-4 flex items-center justify-between gap-4"><h2 className="text-2xl font-black">{lang === 'fa' ? 'تصاویر و نتایج پژوهش' : 'Research visuals & results'}</h2><span className="text-xs font-bold text-slate-400">{article.gallery.length} {lang === 'fa' ? 'تصویر' : article.gallery.length === 1 ? 'figure' : 'figures'}</span></div>
      <div className={`grid gap-4 ${article.gallery.length === 1 ? 'grid-cols-1' : 'md:grid-cols-2'}`}>
        {article.gallery.map((item,index) => {
          const src = typeof item === 'string' ? item : item.src
          const caption = typeof item === 'string' ? title : (item.caption?.[lang] || title)
          const fit = typeof item === 'string' ? '' : item.fit
          return <figure key={`${src}-${index}`} className="group overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="overflow-hidden bg-slate-100 dark:bg-slate-950"><img src={src} alt={caption} loading="lazy" decoding="async" className={`aspect-[4/3] w-full transition duration-500 group-hover:scale-[1.02] ${fit === 'contain' ? 'object-contain p-3' : 'object-cover'}`}/></div>
            <figcaption className="px-4 py-3 text-sm font-semibold leading-6 text-slate-500">{caption}</figcaption>
          </figure>
        })}
      </div>
    </section>}
