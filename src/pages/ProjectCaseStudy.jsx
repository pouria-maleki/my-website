      ...
      {medical ? <>
        ...
        <section className="section-pad"><div className="container-shell">
          <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr]">
            ...
            <div className="case-gallery">{project.gallery.map((src,index)=><img key={src} src={src} alt={index === 0 ? (fa ? 'محیط شبیه‌سازی و گسسته‌سازی چهارراه برای کنترل ترافیک' : 'Traffic-control simulation and intersection discretization') : index === 1 ? (fa ? 'مدل چهارراه در پژوهش کنترل ترافیک' : 'Four-way intersection model used in traffic-control research') : (fa ? 'نمونه ادراک و تشخیص خودرو' : 'Vehicle perception and object-detection example')} loading="lazy"/>)}</div>
          </div>
        </div></section>
